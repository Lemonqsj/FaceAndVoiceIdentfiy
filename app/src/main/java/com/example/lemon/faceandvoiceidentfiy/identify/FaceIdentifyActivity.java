...
import com.example.lemon.faceandvoiceidentfiy.testCameraIdentifyFace;
...
public class FaceIdentifyActivity extends Activity implements OnClickListener {
	...
	private void initUI() {
		TextView title = (TextView) findViewById(R.id.face_idf_txt_title);
		title.setTypeface(FontsUtil.font_yuehei);
		
		mGroupIdTextView = (TextView) findViewById(R.id.face_idf_txt_groupid);
		
		((ImageButton) findViewById(R.id.face_idf_btn_pick)).setOnClickListener(FaceIdentifyActivity.this);
		((ImageButton) findViewById(R.id.face_idf_btn_camera)).setOnClickListener(FaceIdentifyActivity.this);
		((ImageButton) findViewById(R.id.btn_face_idf)).setOnClickListener(FaceIdentifyActivity.this);
	...
	public void onClick(View view) {
		...
		switch (view.getId()) {
		case R.id.face_idf_btn_pick:
			//选择以视频的方式来验证整个

			Intent intent = new Intent(FaceIdentifyActivity.this, testvideoIdentifyFace.class);
...
//			startActivityForResult(intent, REQUEST_PICK_PICTURE);
	...
	protected void handleResult(IdentityResult result) {
		...
		try {
			...
			if(ErrorCode.SUCCESS == resultJson.getInt("ret"))
			{
				...
				FuncUtil.saveObject(FaceIdentifyActivity.this, DemoApp.getmHisList(), DemoApp.HIS_FILE_NAME);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == REQUEST_PICK_PICTURE ) {
			...
			((ImageView) findViewById(R.id.face_idf_img_ifr)).setImageBitmap(mImageBitmap);
		}
	}
